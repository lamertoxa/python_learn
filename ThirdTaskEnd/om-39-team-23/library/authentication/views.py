...
def  signin(request,user=None):
    ...
    if request.method == "POST":
        ...
        logging.warning(f"{password}{email}")
...
def exit(request):
    ...
    logging.warning(f"BEFORE {request.user}")
    logout(request)
    logging.warning(f"AFTER LOG OUT{request.user}")
    return redirect('authentication:start_page')
