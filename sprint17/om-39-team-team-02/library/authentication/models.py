...
class CustomUserManager(BaseUserManager):
    ...
    def create_librarian(self, email, password, **extra_fields):
        ...
        if extra_fields.get('is_staff') is not True:
            raise ValueError(('Librarian must have is_staff=True.'))
    ...
    def create_superuser(self, email, password, **extra_fields):
        ...
        return self.create_user(email, password, first_name="admin",
                                last_name="admin", middle_name="admin", **extra_fields)
...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    created_at = models.DateTimeField(editable=False, auto_now=datetime.datetime.now(), null=True)
    updated_at = models.DateTimeField(auto_now=datetime.datetime.now(), null=True)
    role = models.IntegerField(choices=ROLE_CHOICES, default=0, null=True)
    ...
    def __str__(self):
        ...
        """
        # return f"'id': {self.id}, 'first_name': '{self.first_name}', 'middle_name': '{self.middle_name}', 'last_name': '{self.last_name}', 'email': '{self.email}', 'created_at': {(self.created_at)}, 'updated_at': {(self.updated_at)}, 'role': {self.role}, 'is_active': {self.is_active}, 'password': '{self.password}'"
        return f"id: {self.id}, {self.first_name} {self.middle_name} {self.last_name}"
    ...
    def get_by_super():
        ...
        count = sum([user.is_superuser for user in users])
        return count

    @staticmethod
    def get_by_staff():
        users = CustomUser.objects.all()
        count = sum([user.is_staff for user in users])
        return count
    ...
    def login(email, pas):
        ...
        """
        user_by_email = CustomUser.get_by_email(email)
        print(user_by_email)
        # from hashlib import sha1
        # password = user_by_email['password']
        # _, salt, hashpw = user_by_email['password'].split('$')
        # uncrpt_pas = sha1(salt + password).hexdigest()
        # print(uncrpt_pas)
        # if user_by_email['password'] == pas:
        #     return True
